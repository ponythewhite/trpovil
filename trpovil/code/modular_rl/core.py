...
class StochPolicy(object):
    ...
    def act_with_raw_p_and_log_density(self, ob, stochastic=True):
        ...
        if stochastic:
            hsample = self.probtype.sample(prob)[0]
        else:
            hsample = self.probtype.maxprob(prob)[0]
        
        hLogD = np.log((h*hsample).sum(axis=1))

        return hsample, h[0], hLogD        
...
class CategoricalOneHot(ProbType):
    ...
    def maxprob(self, prob):
        out = np.zeros_like(prob)
#        out[0][prob.argmax(axis=1)] = 1
        out[np.arange(prob.shape[0]), prob.argmax(axis=1)] = 1
        return out
