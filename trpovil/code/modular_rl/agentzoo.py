...
def make_mlps(ob_space, ac_space, cfg):
    ...
    if isinstance(ac_space, Box):
        ...
        outdim = ac_space.n
        probtype = CategoricalOneHot(outdim)
    net = Sequential()
...
class AgentWithPolicy(object):
    ...
    def act_with_raw_p_and_log_density(self, ob_no):
        return self.policy.act_with_raw_p_and_log_density(ob_no, stochastic = self.stochastic)
    def batch_act_with_raw_p_and_log_density(self, ob_nos):
        triples = np.vstack([self.act_with_raw_p_and_log_density(ob_no) for ob_no in ob_nos])
        hsample = triples[:,0]
        hp = triples[:,1]
        hLogD = triples[:,2]
#        hsample = np.vstack(triples[:,0])
#        hp = np.vstack(triples[:,1])
#        hLogD = np.vstack(triples[:,2])
        return hsample, hp, hLogD
    def get_flat(self):
